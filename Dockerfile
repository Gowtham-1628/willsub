...
COPY config/ ./config/
# Keep the legacy fallback path available for production deployments.
COPY config/config.prod.json ./config.json

# Set default environment
ARG NODE_ENV=prod
ENV NODE_ENV=${NODE_ENV}

# Run as the existing node user (UID 1000 matches the host 'ubuntu' user for bind mounts)
RUN mkdir -p logs \
